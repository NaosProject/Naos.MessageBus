...
#endif
{
    ...
    {
        /// <summary>
        /// Launch method that will monitor channels until there is no activity and the <see cref="LaunchConfiguration.TimeToLive" /> has expired or indefinitely if not configured.
        /// </summary>
        ...
        {
            new { messageBusConnectionConfiguration, launchConfig, handlerFactory }.Must().NotBeNull().OrThrowFirstFailure();
            ...
            var reflectionHandlerFactory = handlerFactory as ReflectionHandlerFactory;
            if (reflectionHandlerFactory != null)
            {
